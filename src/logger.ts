...
const stringToRandomBrightHex = (message: string) => {
    const randomBrightColor = Math.floor(Math.random() * 0xFFFF55);
    const hexColor = randomBrightColor.toString(16).padStart(6, '0');
    return chalk.hex('#' + hexColor)(message);
};

const serviceColours = new Map<string, string>();
const serviceColour = (service: string): string => {
    if (!serviceColours.has(service)) serviceColours.set(service, stringToRandomBrightHex(service));
    return serviceColours.get(service)!;
};
                ...
                new winston.transports.Console({
                    format: winston.format.combine(
                        ...
                        winston.format.printf(({ service, level, message, timestamp, ...meta }) => {
                            ...
                            const serviceName = serviceColour((service as string) ?? 'app');
